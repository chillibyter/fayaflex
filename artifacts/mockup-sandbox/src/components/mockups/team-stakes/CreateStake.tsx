...
function Pill({ active, children }: { active?: boolean; children: React.ReactNode }) {
  return (
    <div className={`flex-1 text-center py-2.5 rounded-md text-sm font-semibold ${
      active ? "bg-emerald-500 text-white" : "bg-zinc-800 text-zinc-400"
    }`}>
      {children}
    </div>
  );
...
function GoalCard({ Icon, label, target, unit, active }: { Icon: any; label: string; target: string; unit: string; active?: boolean }) {
  return (
    <div className={`rounded-md p-3 border ${active ? "bg-emerald-500/10 border-emerald-500/40" : "bg-zinc-900 border-zinc-800"}`}>
      <div className={`w-8 h-8 rounded-md flex items-center justify-center mb-2 ${active ? "bg-emerald-500 text-white" : "bg-zinc-800 text-zinc-400"}`}>
...
function StakeOption({ Icon, title, subtitle, badge, badgeColor, active }: { Icon: any; title: string; subtitle: string; badge?: string; badgeColor?: string; active?: boolean }) {
  return (
    <div className={`rounded-md p-3 border flex items-center gap-3 ${active ? "bg-emerald-500/10 border-emerald-500/40" : "bg-zinc-900 border-zinc-800"}`}>
      <div className={`w-9 h-9 rounded-md flex items-center justify-center shrink-0 ${active ? "bg-emerald-500 text-white" : "bg-zinc-800 text-zinc-400"}`}>
        <Icon className="w-4 h-4" />
      </div>
      ...
      <div className={`w-4 h-4 rounded-sm border-2 ${active ? "bg-emerald-500 border-emerald-500" : "border-zinc-700"}`} />
    </div>
  );
}
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-zinc-100 font-['Inter'] flex flex-col">
      ...
      <div className="px-4 pb-32 space-y-5">
        <div>
          ...
          <div className="rounded-md bg-zinc-900 border border-zinc-800 p-3 flex items-center gap-3">
            <div className="w-10 h-10 rounded-md bg-gradient-to-br from-emerald-500 to-green-700 flex items-center justify-center text-white font-bold">
              FS
            </div>
            ...
            <button className="text-xs text-emerald-400 font-semibold">Change</button>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            ...
            <div className="text-sm font-bold text-emerald-400 tabular-nums">35,000 steps</div>
          </div>
          <div className="rounded-md bg-zinc-900 border border-zinc-800 p-4">
            <div className="relative h-1.5 bg-zinc-800 rounded-sm">
              <div className="absolute inset-y-0 left-0 bg-emerald-500 rounded-sm" style={{ width: "55%" }} />
              <div className="absolute -top-1.5 w-4 h-4 rounded-full bg-emerald-500 border-2 border-zinc-950" style={{ left: "calc(55% - 8px)" }} />
            </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-zinc-950 border-t border-zinc-800 px-4 py-3 pb-6">
        <button className="w-full bg-emerald-500 text-white font-semibold py-3 rounded-md text-base">
